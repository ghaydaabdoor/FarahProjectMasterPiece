...
namespace MasterPiece.Controllers
{
    public class BookingsController : Controller
    {
        ...
        public ActionResult Create(int? id)
        {
            ...
            if (user != null)
            {
                string redirectUrl = Url.Action("CreateVenueFormView", "Bookings", new { id = id });
        ...
        public ActionResult CreateVenueFormView(int id)
        {
            int userId = (int)Session["UserId"];
            var user = db.Users.FirstOrDefault(u => u.UserId == userId);
        ...
        public ActionResult Create([Bind(Include = "BookingId,UserId,FullName,Email,Phone,BookingDate,BookingTimeFrom,SpecialOrder,IsApproved,BookingTimeTo,VenueId")] Booking booking)
        {
            ...
            if (conflictingBooking != null)
            {
                ...
                booking.WeddingVenue = db.WeddingVenues.FirstOrDefault(v => v.VenueId == booking.VenueId);
                return View("CreateVenueFormView", booking);  // Explicitly return the correct view
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                return RedirectToAction("CreateVenueFormView", "Bookings", new { id = booking.VenueId });  // Redirect to a different page on successful booking.
            }

            ViewBag.UserId = new SelectList(db.Users, "UserId", "FirstName", booking.UserId);
            booking.WeddingVenue = db.WeddingVenues.FirstOrDefault(v => v.VenueId == booking.VenueId);
            return View("CreateVenueFormView", booking);
        }
        ...
        public ActionResult CreateFarmForm(int? id)
        {
            ...
            if (user != null)
            {
                string redirectUrl = Url.Action("CreateFarmFormView", "Bookings", new { id = id });
                return Json(new { redirect = redirectUrl }, JsonRequestBehavior.AllowGet);
            }
        ...
        public ActionResult CreateFarmFormView(int id)
        {
            int userId = (int)Session["UserId"];
            var user = db.Users.FirstOrDefault(u => u.UserId == userId);
        ...
        public ActionResult CreateFarmForm([Bind(Include = "BookingId,UserId,FullName,Email,Phone,BookingDate,BookingTimeFrom,SpecialOrder,IsApproved,BookingTimeTo,FarmId")] Booking booking)
        {
            ...
            {
                ModelState.AddModelError("BookingTimeFrom", "هذا الوقت محجوز بالفعل. يرجى اختيار وقت آخر");
                booking.Farm = db.Farms.FirstOrDefault(v => v.FarmId == booking.FarmId);
                return View("CreateFarmFormView", booking);  // Explicitly return the correct view
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                TempData["BookingSuccess"] = "تم حجزك بنجاح";
                TempData.Keep("BookingSuccess");
                return RedirectToAction("CreateFarmFormView", "Bookings", new { id = booking.FarmId });
            }
        ...
        public ActionResult CreatePhotogForm([Bind(Include = "BookingPhotogId,UserId,FullName,Email,Phone,BookingDate,BookingTimeFrom,SpecialOrder,IsApproved,BookingTimeTo,PhotographerId")] BookingPhotographer bookingphotog)
        {
            ...
            {
                ModelState.AddModelError("BookingTimeFrom", "هذا الوقت محجوز بالفعل. يرجى اختيار وقت آخر");
                bookingphotog.Photographer = db.Photographers.FirstOrDefault(v => v.PhotographerId == bookingphotog.PhotographerId);
                return View("CreatePhotogFormView", bookingphotog);  // Explicitly return the correct view
            }

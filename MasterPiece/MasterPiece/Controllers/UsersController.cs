...
namespace MasterPiece.Controllers
{
    public class UsersController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditProfile([Bind(Include = "UserId,FirstName,LastName,Phone,Address")] User model, HttpPostedFileBase ProfileImage)
        {
            if (ModelState.IsValid)
            {
                ...
                if (existingUser != null)
                {
                    ...
                    {
                        var filePath = ProfileImage.FileName; // The full path will be something like "C:\Users\Orange\Desktop\file.jpg"

                        // Use Split to get the file name
                        var fileName = filePath.Split('\\').Last();  // This will give you just the file name, e.g., "testimonials-5.jpg"

                        // Define the path to save the image (e.g., in the ~/Content/img/testimonials folder)
                        var path = Path.Combine(Server.MapPath("~/Content/img/testimonials/"), fileName);

                        // Save the image to the server
                        ProfileImage.SaveAs(path);

                        // Update the user's profile picture path (you now store just the file name)
                        existingUser.PersonalImage = fileName;
                    }

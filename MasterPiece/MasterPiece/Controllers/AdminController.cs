...
namespace MasterPiece.Controllers
{
    public class AdminController : Controller
    {
        ...
        public ActionResult EditVenue(WeddingVenueViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (venue != null)
                {
                    ...
                    venue.Discount = viewModel.Venue.Discount;
                ...
                if (venueDetails != null)
                {
                    ...
                    venueDetails.PriceDetails = viewModel.VenueDetails.PriceDetails;
        ...
        public ActionResult EditFarm(FarmViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (farm11 != null)
                {
                    ...
                    farm11.Discount = viewModel.farm.Discount;
        ...
        // Invitation Cards
        public ActionResult ShowInvitationCards()
        {
            return View(db.InvitationCards.ToList());
        }

        public ActionResult CreateInvitationCards()
        {
            return View();
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult CreateInvitationCards([Bind(Include = "InvitationCardsId,CardName,Price,ImageUrl")] InvitationCard card)  // Bind the properties of the Food model
        {
            if (ModelState.IsValid)
            {
                db.InvitationCards.Add(card);
                db.SaveChanges();
                return RedirectToAction("ShowInvitationCards");
            }

            return View(card);
        }
        public ActionResult EditInvitationCards(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            InvitationCard card = db.InvitationCards.Find(id);  // Find the food item by ID
            if (card == null)
            {
                return HttpNotFound();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditInvitationCards([Bind(Include = "InvitationCardsId,CardName,Price,ImageUrl")] InvitationCard card)  // Bind the properties of the Food model
        {
            if (ModelState.IsValid)
            {
                db.Entry(card).State = EntityState.Modified;  // Modify the existing food item
                db.SaveChanges();
                return RedirectToAction("ShowInvitationCards");
            }
            return View(card);
        ...
        // GET: Food/Delete/5
        public ActionResult DeleteInvitationCards(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
        ...
        // POST: Food/Delete/5
        [HttpPost, ActionName("DeleteInvitationCards")]
        [ValidateAntiForgeryToken]
        public ActionResult DeleteConfirmedCards(int id)
        {
            InvitationCard card = db.InvitationCards.Find(id);  // Find the food item by ID
            db.InvitationCards.Remove(card);  // Remove the food item from the database
            db.SaveChanges();
            return RedirectToAction("ShowInvitationCards");
        }
        ...
        public ActionResult ShowVenuesBooking()
        {
            // Fetch the list of distinct venues from the database to avoid duplicates
            var venues = db.WeddingVenues.ToList();

            // Pass the list to the view using ViewBag
            ViewBag.Venues = venues;

            // Fetch the bookings with VenueId
            var bookings = db.Bookings
                             .Include(b => b.User)
                             .Include(b => b.WeddingVenue)
                             .Where(b => b.VenueId != null); // Filter out bookings without a VenueId
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AddVenueBooking([Bind(Include = "BookingId,FullName,BookingDate,BookingTimeFrom,SpecialOrder,IsApproved,BookingTimeTo,VenueId")] Booking booking)
        {
            ...
            if (conflictingBooking != null)
            {
                ...
                return RedirectToAction("ShowVenuesBooking"); // Stay on the same view and reload data
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                return RedirectToAction("ShowVenuesBooking");  // Redirect to the main page
            }

            // Re-fetch the data in case of an error and reload the same view
            ViewBag.Venues = db.WeddingVenues.ToList();
            var bookings = db.Bookings.Include(b => b.User).Include(b => b.WeddingVenue).Where(b => b.VenueId != null);
            return View("ShowVenuesBooking", bookings.ToList()); // Return to the same view
        }



        public ActionResult ShowFarmsBooking()
        {
            // Fetch the list of distinct venues from the database to avoid duplicates
            var farms = db.Farms.ToList();
            ...
            var bookings = db.Bookings
                              .Include(b => b.User)
                              .Include(b => b.WeddingVenue)
                              .Where(b => b.FarmId != null); // Filter out bookings without a VenueId
            return View(bookings.ToList());
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AddFarmBooking([Bind(Include = "BookingId,FullName,BookingDate,BookingTimeFrom,SpecialOrder,IsApproved,BookingTimeTo,FarmId")] Booking booking)
        {
            if (booking.BookingDate == null)
            {
                // Add error message to the ModelState
            ...
            if (ModelState.IsValid)
            {
                db.Bookings.Add(booking);
                db.SaveChanges();
                return RedirectToAction("ShowFarmsBooking");  // Redirect to the main page
            }

            // Re-fetch the data in case of an error and reload the same view
            ViewBag.Farms = db.Farms.ToList();
            var bookings = db.Bookings.Include(b => b.User).Include(b => b.FarmId).Where(b => b.FarmId != null);
            return View("ShowFarmsBooking", bookings.ToList()); // Return to the same view
        }



        public ActionResult ShowClothsBooking()
        {
            // Fetch the list of distinct venues from the database to avoid duplicates
            var cloth = db.Clothes.ToList();

            // Pass the list to the view using ViewBag
            ViewBag.Cloth = cloth;

            var bookings = db.BookingCloths.Include(b => b.Cloth);
            return View(bookings.ToList());
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AddClothBooking([Bind(Include = "BookingClothId,FullName,BookingDateFrom,BookingDateTo,IsApproved,ClothId")] BookingCloth bookingcloth)

        {
            ...
            if (conflictingBooking != null)
            {
                ...
                return RedirectToAction("ShowClothsBooking");
            }
            ...
            // Re-fetch the data in case of an error and reload the same view
            ViewBag.Cloth = db.Clothes.ToList();
            var bookings = db.BookingCloths.Include(b => b.User).Include(b => b.ClothId).Where(b => b.ClothId != null);
            return View("ShowClothsBooking", bookings.ToList()); // Return to the same view
        }


        public ActionResult ShowPhotographersBooking()
        {
            var photographers = db.Photographers.ToList();
            ViewBag.Photographers = photographers;


            var photographersBookings = db.BookingPhotographers.Include(b => b.Photographer);
            return View(photographersBookings.ToList());
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AddPhotographersBooking([Bind(Include = "BookingPhotogId,FullName,BookingDate,BookingTimeFrom,SpecialOrder,IsApproved,BookingTimeTo,PhotographerId")] BookingPhotographer bookingPhotographer)
        {
            if (bookingPhotographer.BookingDate == null)
            {
                ModelState.AddModelError("BookingDate", "يرجى تحديد تاريخ الحجز");
            }
            ...
            var conflictingBooking = db.BookingPhotographers
                .Where(b => b.PhotographerId == bookingPhotographer.PhotographerId
                            && b.BookingDate == bookingPhotographer.BookingDate
                            && ((bookingPhotographer.BookingTimeFrom >= b.BookingTimeFrom && bookingPhotographer.BookingTimeFrom < b.BookingTimeTo)
                                || (bookingPhotographer.BookingTimeTo > b.BookingTimeFrom && bookingPhotographer.BookingTimeTo <= b.BookingTimeTo)
                                || (bookingPhotographer.BookingTimeFrom <= b.BookingTimeFrom && bookingPhotographer.BookingTimeTo >= b.BookingTimeTo)))
                .FirstOrDefault();

            if (conflictingBooking != null)
            {
                ModelState.AddModelError("BookingTimeFrom", "هذا الوقت محجوز بالفعل. يرجى اختيار وقت آخر");
                return RedirectToAction("ShowPhotographersBooking"); // Stay on the same view and reload data
            }

            if (ModelState.IsValid)
            {
                db.BookingPhotographers.Add(bookingPhotographer);
                db.SaveChanges();
                return RedirectToAction("ShowPhotographersBooking");  // Redirect to the main page
            }

            // Re-fetch the data in case of an error and reload the same view
            ViewBag.Photographers = db.Photographers.ToList();
            var bookingPhotog = db.BookingPhotographers.Include(b => b.User).Include(b => b.Photographer).Where(b => b.PhotographerId != null);
            return View("ShowPhotographersBooking", bookingPhotog.ToList()); // Return to the same view
        }
        ...
        [ValidateAntiForgeryToken]
        public ActionResult FeedbackDeleteConfirmed(int id)
        {
            Feedback feedback = db.Feedbacks.Find(id);
